...
def editBrewery(brewery_id):
	breweryToEdit = session.query(
		Brewery).filter_by(id=brewery_id).one()
	if breweryToEdit.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this brewery. Please create your own brewery to order to edit.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		session.edit(breweryToEdit)
		flash('Brewery Successfully Edited %s' % breweryToEdit.name)
		session.commit()
		return redirect(url_for('showBreweries', brewery_id=brewery_id))
	else:
		return render_template('editBrewery.html', brewery=breweryToEdit)
...
def deleteBrewery(brewery_id):
	...
	if breweryToDelete.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to delete this brewery. Please create your own brewery in order to delete.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
...
def newBeerName(brewery_id):
	...
	brewery = session.query(Brewery).filter_by(id=brewery_id).one()
	if request.method == 'POST':
		newBeer = BeerName(name=request.form['name'],
...
def editBeerName(brewery_id, beer_id):
	# if 'username' not in login_session:
	#   return redirect('/login')
	beerToEdit = session.query(BeerName).filter_by(id=beer_id).one()
	if beerToEdit.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this beer. Please create your own to edit.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		...
		if request.form['price']:
			beerToEdit.price = request.form['price']
...
def deleteBeerName(brewery_id, beer_id):
	...
	if beerToDelete.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to delete this beer. Please create your own to delete.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
